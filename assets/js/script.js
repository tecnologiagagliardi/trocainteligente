  ...
  const updateMap = (latitude, longitude) => {
    if (!map) {
      ...
      L.marker([latitude, longitude])
        .addTo(map)
        .bindPopup(`O Tanque e a Propulsora estão aqui!<br><br>Lat: ${latitude.toFixed(6)}<br>Lng: ${longitude.toFixed(6)}`)
        .openPopup();
    } else {
  ...
  document.getElementById('capture-photo').addEventListener('click', async () => {
    ...
    try {
      ...
      info.innerHTML = `
        ...
        <strong>N° Patrimônio Propulsora:</strong> ${propNumber || 'Não fornecido'}<br>
        <strong>N° Patrimônio Tanque:</strong> ${tankNumber || 'Não fornecido'}<br>
        <strong>Capacidade do Tanque:</strong> ${capacity || 'Não fornecido'} Litros<br>
        <strong>Quantidade no Tanque:</strong> ${quantity || 'Não fornecido'} Litros<br>
